//get_at_least_subtask1_and_2_for_both_tasks
// matric number: A0199806L
// name: Goh Kheng Xi, Jevan
// lab group: B04
// lab group TA: Wu Biao
#include <stdio.h>
#include <iostream>
#include <vector>
#include <queue>
#include <unordered_set>
#include <vector>
#include<map>
#include <unordered_map>

using namespace std; //reusing my bridge codes

struct Comparator{
    public:
        bool operator()(vector<long long> a, vector<long long> b){
            if (a[0] > b[0]){
                return true;
            }
            return false;
        }
};

int main(){
    ios::sync_with_stdio(false); cin.tie(NULL);
    long long n, m;
    unordered_map<long long, vector<pair<long long, long long>>> AM;
    unordered_set<long long> visited;
    cin >> n >> m;
    for (long long i = 0; i < m; ++i){
        long long a, b, c;
        cin >> a >> b >> c;
        pair<long long, long long> bridge_type;
        bridge_type.first = b;
        bridge_type.second = c;
        if (AM.find(a) == AM.end()){
            vector<pair<long long, long long>> neighbours;
            neighbours.push_back(bridge_type);
            AM.insert({a, neighbours});
        } else {
            AM[a].push_back(bridge_type);
        }
        pair<long long, long long> reverse_bridge_type;
        reverse_bridge_type.first = a;
        reverse_bridge_type.second = c;
        if (AM.find(b) == AM.end()){
            vector<pair<long long, long long>> neighbours;
            neighbours.push_back(reverse_bridge_type);
            AM.insert({b, neighbours});
        } else {
            AM[b].push_back(reverse_bridge_type);
        }
    }
    
    priority_queue<vector<long long>, vector<vector<long long>>, Comparator> all_paths;
    vector<long long>initial_path;
    initial_path.push_back(0);
    initial_path.push_back(0);
    visited.insert(0);
    all_paths.push(initial_path);
    vector<vector<long long>>min_paths;
    while (!all_paths.empty()){
        vector<long long>currentPath = all_paths.top();
        long long cur_pos = all_paths.top()[all_paths.top().size()-1];
        if (cur_pos == n-1){
            if (min_paths.size() == 0){
                min_paths.push_back(currentPath);
            } else if (min_paths[0][0] < currentPath[0]){
                min_paths.push_back(currentPath);
            } else {
                break;
            }
            all_paths.pop();
        }
        else{
            all_paths.pop();
            for (auto b: AM[cur_pos]){ 
                if (visited.find(b.first) == visited.end()){
                    vector<long long>currentPath_copy = currentPath;
                    currentPath_copy.push_back(b.first);
                    currentPath_copy[0] += b.second;
                    visited.insert(b.first);
                    all_paths.push(currentPath_copy);
                }
                
            }
        }
        

    }
    unordered_map<long long, long long>answers;

    for (auto paths: min_paths){
        for (long long step = 1; step < paths.size(); ++ step){
            if (answers.find(paths[step]) == answers.end()){
                answers.insert({paths[step], 1});
            } else {
                answers[paths[step]] += 1;
            }
        }
    }

    for (long long i = 0; i <n; ++i){
        if (answers.find(i) == answers.end()){
            cout << "-1 ";
        } else {
            cout << answers[i] << " ";
        }
    }
    
    


}