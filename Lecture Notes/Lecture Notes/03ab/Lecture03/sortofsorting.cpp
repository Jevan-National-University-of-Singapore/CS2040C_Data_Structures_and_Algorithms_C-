// there is a subtle bug in this code, submitting it verbatim will give you Wrong Answer
...
int main() { // the whole thing is O(N log N)
  ...
  while (cin >> n, n) { // probably new "technique" to many of you: we can chain two C++ commands into one line using a comma, the last value (n here) is used as the test condition for the while loop, the while loop stops when n == 0
    ...
    sort(names.begin(), names.end(), [](string a, string b) { // using C++11 lambda function, note that you have to compile this file using g++ -std=c++11 sortofsorting.cpp (obviously -std=c++17 works too)
      if (a[0] != b[0]) return a[0] < b[0];
      return a[1] < b[1];
    }); // O(N log N), do you realize why length of string doesn't matter here because we only compare the first two characters of string a and b?
    if (!FirstTestCase) cout << "\n";
    FirstTestCase = false;
